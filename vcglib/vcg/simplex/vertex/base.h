...
public:
	...
	// 检查顶点是否被删除
    bool IsD() const {return (this->cFlags() & DELETED) != 0;} ///  checks if the vertex is deleted检查是否这个点被删除了

	// 检查顶点是否可读
    bool IsR() const {return (this->cFlags() & NOTREAD) == 0;} ///  checks if the vertex is readable检查这个点是否可读
